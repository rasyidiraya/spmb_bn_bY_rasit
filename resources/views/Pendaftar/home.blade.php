 ...
 <main class="main">
          ...
          <div class="col-md-6">
            <div class="highlight-item">
              ...
              @foreach($allGelombang as $gelombang)
                ...
                @endphp
                <div class="gelombang-item {{ $isActive ? 'active' : 'inactive' }}">
                  <strong>{{ $gelombang->nama }}</strong><br>
                  {{ \Carbon\Carbon::parse($gelombang->tgl_mulai)->format('M') }} - {{ \Carbon\Carbon::parse($gelombang->tgl_selesai)->format('M Y') }}<br>
                  Rp {{ number_format($gelombang->biaya_daftar, 0, ',', '.') }}
                  @if($isActive) <span class="gelombang-status">● AKTIF</span> @endif
                </div>
              @endforeach
            </div>
          </div>
    ...
    <section id="timeline" class="section">
      <div class="container section-title">
        ...
        <p>Ikuti langkah-langkah pendaftaran berikut secara berurutan</p>
      </div>
      ...
      <div class="container">
          ...
          @php
            ...
            if ($pendaftar && in_array($pendaftar->status, ['ADM_REJECT', 'PAYMENT_REJECT'])) {
              $steps = [
                ...
                ['name' => 'Verifikasi Admin', 'done' => false],
                ['name' => 'Bayar Biaya', 'done' => false],
                ['name' => 'Selesai', 'done' => false]
              ];
            } else {
              $berkasCount = $pendaftar ? $pendaftar->berkas->count() : 0;
              $steps = [
                ['name' => 'Isi Formulir', 'done' => $pendaftar ? true : false],
                ['name' => 'Upload Berkas', 'done' => $berkasCount >= 3],
                ['name' => 'Verifikasi Admin', 'done' => $pendaftar && in_array($pendaftar->status, ['ADM_PASS', 'PAYMENT_PENDING', 'PAID'])],
                ['name' => 'Bayar Biaya', 'done' => $pendaftar && in_array($pendaftar->status, ['PAYMENT_PENDING', 'PAID'])],
                ['name' => 'Selesai', 'done' => $pendaftar && $pendaftar->status == 'PAID']
              ];
            }
          @endphp
        @else
        ...
        <div class="d-flex justify-content-between align-items-center timeline-steps">
          @foreach($steps as $index => $step)
            <div class="timeline-step">
              <div class="timeline-step-icon {{ $step['done'] ? 'completed' : 'pending' }}">
                @if($step['done'])
              ...
              <small class="timeline-step-text {{ $step['done'] ? 'completed' : 'pending' }}">{{ $step['name'] }}</small>
            </div>
